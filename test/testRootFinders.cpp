...
namespace anpi {
    namespace test {
        ...
        /// x^4 + x^3 + 7x^2 + 9x - 18
        //template<typename T>
        boost::math::tools::polynomial<T> t1{{1.0,1.0,7.0, 9.0,-18.0}}; /// raices reales y complejas x=1,\:x=3i,\:x=-3i,\:x=-2

        /// Second testing function for roots (x+1)(x²-9)(x+4)
        ///x^4+3x^3-7x^2-27x-18
        template<typename T>
        boost::math::tools::polynomial<T> t2{{1.0,3.0,-7.0, -27.0,-18.0}}; /// 4 raices reales x=-1,\:x=-2,\:x=-3,\:x=3

        /// Third testing function for roots (x-2)(x²+16)(x+9)
        template<typename T>
        ///x^4+7x^3-2x^2+112x-288\:
        boost::math::tools::polynomial<T> t3{{1.0,7.0,-2.0,112.0,-288.0}};  /// raices reales y complejas x=2,\:x=4i,\:x=-4i,\:x=-9
