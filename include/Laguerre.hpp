    ...
    */
    ...
    T laguerre(const boost::math::tools::polynomial<T>& a, std::complex<T> x, const T eps) {
        ...
        for (int i = 1; i<= MAXIT; i++) {
            ...
            err *= eps;
            if (std::abs(b) <= err) return x;       //We are on the root
            g = d/b;
            g2 = g*g;
            h = (g2-(T(2.0)* (f/ b)));
            sq = std::sqrt((m - 1)*((m* h)- g2));
            gp = g+sq;
            gm = g-sq;
            abp = std::abs(gp);
            abm = std::abs(gm);
            if (abp<abm)
                gp = gm;
            dx = ((fmax(abp, abm) > 0.0 ? (std::complex((float) m, 0.0)/ gp)
                                        : (1 + abx * std::complex(cos((float) i) * sin((float) i)))));
            x1 = x-dx;
            if (x.real() == x1.real() && x.imag() == x1.imag()) return x;    //converge
            if (i % MT) x = x1;
            else x = x-(frac[i / MT] * dx);
        }
