...
namespace anpi {
     ...
     */
    template<typename T>
    T rootInterpolation(const boost::math::tools::polynomial<T>& funct,T xl,T xu,const T eps) {

        T xr=xl;    //hay que iniciar con algo válido
        T fl = funct.evaluate(xl);
        T fu = funct.evaluate(xu);
        T ea=T ( );
        ...
        for(int i =std::numeric_limits<T>::digits; i > 0; --i) {
            T xrold(xr);   //para cálculo de error  ///xrold es una funcion??? si lo es, poner el evaluate
            xr=xu - fu * (xl-xu)/(fl - fu);
            T fr =funct.evaluate(xr);
